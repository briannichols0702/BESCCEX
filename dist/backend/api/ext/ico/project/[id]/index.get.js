...
exports.default = async (data) => {
    const { params } = data;
    const { id } = params;
    const project = (await db_1.models.icoProject.findByPk(id, {
        include: [
            {
    ...
    if (!project) {
        throw (0, error_1.createError)(404, "Project not found");
    }
    const tokens = project.icoTokens.map((token) => {
        const phases = token.icoPhases.map((phase) => {
            const totalContributionAmount = phase.icoContributions.reduce((sum, contribution) => sum + contribution.amount, 0);
            const contributionPercentage = (totalContributionAmount / token.totalSupply) * 100;
