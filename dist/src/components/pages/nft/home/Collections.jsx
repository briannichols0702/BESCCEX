...
const TopCollections = () => {
    const { topCollections } = (0, nft_1.useNftStore)();
    return (<div className="w-full px-16 py-12 relative">
      {/* Header Section */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl lg:text-4xl font-bold text-muted-800 dark:text-muted-200 z-10">
          Today&apos;s Top Collections
        </h2>
        <link_1.default href="/nft/collections" passHref className="z-10">
          <button className="flex items-center space-x-1 bg-muted-100/50 dark:bg-muted-500/50 text-muted-800 dark:text-muted-100 rounded-md px-4 py-2 text-sm font-medium transition hover:bg-muted-200/50 dark:hover:bg-muted-600/50">
            <span>Analytics</span>
            <react_2.Icon icon="akar-icons:arrow-up-right" className="w-4 h-4"/>
          </button>
        </link_1.default>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {topCollections.length > 0 ? (topCollections.map((collection, index) => (<link_1.default key={collection.id} href={`/nft/collection/${collection.id}`} className="flex items-center justify-between py-4 px-4 border-b border-muted-300 dark:border-muted-700 hover:bg-muted-100 dark:hover:bg-muted-700 transition rounded-xs">
              ...
              <div className="flex items-center space-x-4">
                ...
                {/* Collection Image */}
                <img src={collection.image || "/default-placeholder.png"} alt={collection.name} className="z-10 w-10 h-10 rounded-full border border-muted-300 dark:border-muted-600"/>
            ...
            </link_1.default>))) : (<p className="text-muted-600 dark:text-muted-400 text-center w-full col-span-2">
            No top collections available at the moment.
          </p>)}
      </div>
    </div>);
};
exports.default = TopCollections;
