...
const OrderInputBase = ({ type, side }) => {
    ...
    return (<div className="flex flex-col gap-2 justify-between h-full w-full">
     ...
     onChange={(e) => setAmount(parseFloat(e.target.value))} min={minAmount} max={maxAmount} step={minAmount > 0 ? minAmount : Math.pow(10, -getPrecision("amount"))}/>
      ...
      <div className="flex flex-col gap-1">
        <Button_1.default type="button" color={(profile === null || profile === void 0 ? void 0 : profile.id) ? (side === "BUY" ? "success" : "danger") : "muted"} animated={false} className="w-full" shape={"rounded-xs"} disabled={!(profile === null || profile === void 0 ? void 0 : profile.id) ||
            (type !== "MARKET" && price === 0) ||
            amount === 0 ||
            !amount} onClick={() => {
